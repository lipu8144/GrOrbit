...
describe("JOURNEY: ordering a portion (half plate / full plate)", () => {
  it("customer picks a portion in the sheet and pays that price", async () => {
    ...
    expect(order.total).toBe(449);
  }, 20000);
});

describe("JOURNEY: ordering with add-ons", () => {
  it("extras stack on top of the base price and appear on the ticket", async () => {
    renderAt("/r/spice-junction?fresh");
    await screen.findByText(/Scan & order/i, {}, { timeout: 4000 });

    // The demo Classic Veg Burger (₹149) has add-ons: Extra cheese +₹30, Add egg +₹20.
    // Items with add-ons show a CUSTOMISE path rather than a bare ADD.
    const card = (await screen.findByText(/Classic Veg Burger/i)).closest("div").parentElement;
    fireEvent.click(within(card).getByRole("button", { name: /ADD/i }));

    // the sheet opens — tick one extra, then add
    expect(await screen.findByText(/Add extras/i)).toBeInTheDocument();
    fireEvent.click(screen.getByText("Extra cheese"));
    fireEvent.click(screen.getByRole("button", { name: /^Add ·/i }));

    fireEvent.click(screen.getByText(/View cart/i));
    fireEvent.change(screen.getByPlaceholderText("e.g. Aarav"), { target: { value: "Addon Tester" } });
    fireEvent.click(screen.getByText(/Place order/i));
    await screen.findByText(/Your token number/i);

    // 149 base + 30 extra = 179, and the extra is named for the kitchen
    const order = getPlacedOrders()[0];
    expect(order.items[0].price).toBe(179);
    expect(order.items[0].name).toMatch(/Classic Veg Burger \+ Extra cheese/);
    expect(order.total).toBe(179);
  }, 20000);
});
